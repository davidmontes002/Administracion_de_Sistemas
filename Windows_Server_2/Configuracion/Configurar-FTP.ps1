function Registrar-Grupo-FTP {
    ...
    Write-Host "3) Inicializar Ambos Entornos"
...
function Registrar-Alumno-FTP {
    ...
    do {
        $FTPPassword = Read-Host "Ingresar una contrasena (Min. 8 char, Mayus, Minus, Num)"
        if ($FTPPassword -notmatch $regex) {
            Write-Host "Contrasena no valida." -ForegroundColor Red
        } else { break }
    ...
    Write-Host "> 3. Creando Uniones de Directorio..." -ForegroundColor Cyan
    ...
    Write-Host "> 4. Aplicando Permisos Automatizados..." -ForegroundColor Cyan
    ...
    Write-Host "> 5. Refrescando reglas de IIS..." -ForegroundColor Cyan
    Import-Module WebAdministration
    ...
    Write-Host "+ Listo! Usuario $FTPUserName configurado." -ForegroundColor Green
    Pause
}
